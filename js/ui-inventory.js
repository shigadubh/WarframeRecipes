...
function renderAddItemList() {
    ...
    if (search) items = items.filter(c => normalize(c.displayName).includes(search) || normalize(c.parentName).includes(search));
    ...
    display.forEach(comp => {
        const inInv = inv.hasItem(comp.id);
        const qty = inv.getQty(comp.id);
        const div = document.createElement('div');
        div.className = `add-item-option${inInv ? ' in-inventory' : ''}`;
        div.innerHTML = `
            <div style="min-width:0;flex:1;">
                <div class="add-item-option-name">${comp.displayName}${inInv ? ` <small style="color:var(--success);">(x${qty})</small>` : ''}</div>
                <div class="add-item-option-sub">📦 ${comp.parentName}</div>
            </div>
            <button class="add-item-option-btn ${inInv ? 'remove' : 'add'}">${inInv ? 'Remover' : 'Adicionar'}</button>`;

        div.querySelector('button').addEventListener('click', e => {
            e.stopPropagation();
            if (inInv) { inv.removeItem(comp.id); showToast(`${comp.displayName} removido`, 'error'); }
